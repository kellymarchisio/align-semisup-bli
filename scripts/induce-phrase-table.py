...
def write_phrase_table(x, z, words_x, words_z, temperature, size, batch_size, min_prob, device, f, sep):
    with torch.no_grad():
        ...
        for i in range(0, x.shape[0], batch_size):
            ...
            for k in range(i, j):
                for prob, invprob, l in zip(np_probs[k-i], np_invprobs[k-i], np_ind[k-i]):
                    ...
                    print('{0} {6} {1} {6} {2} {3} {4} {5} {6} {6} {6} {6}'.format(
                        words_x[k].replace('&#32;', ' '), words_z[l].replace('&#32;', ' '), prob, invprob, lexprob, invlexprob, sep), file=f)
...
def main():
    ...
    # Kelly changed separator on 30 Jan 2021 b/c official Artetxe/Dinu embeddings
    # actually have ||| and ||||| as words in the vocabulary.
    parser.add_argument('--sep', default='|||', help='default separator for output file.')
    ...
    if args.src2trg is not None:
        ...
        write_phrase_table(x, z, src_words, trg_words, t, args.size, args.batch, args.min_prob, device, f, args.sep)

    if args.trg2src is not None:
        f = open(args.trg2src, mode='w', encoding=args.encoding, errors='surrogateescape')
        write_phrase_table(z, x, trg_words, src_words, t, args.size, args.batch, args.min_prob, device, f, args.sep)
