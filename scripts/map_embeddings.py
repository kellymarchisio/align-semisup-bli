# This file is originally from Vecmap (https://github.com/artetxem/vecmap.git) 
# Kelly Marchisio added some small edits, Jan 2021.
...
def main():
    ...
    parser.add_argument('--max_embs', default=0, type=int, help='Maximum embeddings to read from file. 0 == read all.')
    # Kelly Marchisio changed separator on 30 Jan 2021 to ||| b/c official Artetxe/Dinu embeddings
    # actually have ||| and ||||| as words in the vocabulary.
    # Kelly Marchisio changed again to None on 24 March 2021 b/c facebook dictionaries
    # are inconsistently separated by space or tab.
    parser.add_argument('--sep', default=None, help='default separator for output file.')
    ...
    src_words, x = embeddings.read(srcfile, threshold=args.max_embs, dtype=dtype)
    trg_words, z = embeddings.read(trgfile, threshold=args.max_embs, dtype=dtype)
    ...
    if args.init_unsupervised:
        ...
        for line in f:
            src, trg, *others = line.split(args.sep)
            try:
                src = src.strip()
                trg = trg.strip()
